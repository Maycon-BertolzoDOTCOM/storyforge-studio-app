...
describe("updater fixture server", () => {
  ...
  it("serves a local artifact file as the updater installer", async () => {
    const root = await mkdtemp(join(tmpdir(), "storyforge-updater-fixture-"));
    const artifactPath = join(root, "StoryForge-release-beta-win-setup.exe");
    ...
    try {
      ...
      expect(metadata.platforms?.win?.artifacts?.installer?.name).toBe("StoryForge-release-beta-win-setup.exe");
  ...
  it("serves launcher payload bytes from a real archive path", async () => {
    ...
    const payloadPath = join(root, "StoryForge-release-preview-payload.zip");
    ...
    try {
      ...
      expect(metadata.platforms?.mac?.artifacts?.payload?.name).toBe("StoryForge-release-preview-payload.zip");
  ...
  it("serves a local launcher payload artifact file", async () => {
    const root = await mkdtemp(join(tmpdir(), "storyforge-updater-payload-fixture-"));
    const payloadPath = join(root, "StoryForge-release-beta-win-payload.7z");
