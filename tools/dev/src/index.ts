...
import { createSidecarLaunchEnv, requestJsonIpc } from "@storyforge-app/sidecar";
import {
...
async function buildDesktop(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  await logHandle.write(`\n[tools-dev] building @storyforge-app/desktop at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@storyforge-app/desktop", "build"], process.env);
...
async function ensureDaemonCliBuild(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  ...
  await logHandle.write(`\n[tools-dev] building @storyforge-app/daemon because ${reason} at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@storyforge-app/daemon", "build"], process.env);
  await runLoggedCommand({
    args: invocation.args,
    command: invocation.command,
    cwd: config.workspaceRoot,
...
async function runForeground(config: ToolDevConfig, appName: string | undefined, options: CliOptions) {
  ...
  await new Promise<void>((resolveDone) => {
    const shutdown = () => {
      ...
      process.stderr.write("\nStopping StoryForge dev server...\n");
...
function addPortOptions(command: ReturnType<typeof cli.command>) {
  return command
    ...
    .option("--prod", "use production build (requires pnpm --filter @storyforge-app/web build first)");
}
