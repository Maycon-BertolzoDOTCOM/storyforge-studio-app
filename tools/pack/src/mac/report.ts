...
export async function collectMacSizeReport(
  ...
  return {
    ...
    tracked: {
      ...
      webNextCacheBytes: await sizePathBytes(join(appNodeModulesRoot, "@storyforge", "web", ".next", "cache")),
      webPackageBytes: await sizePathBytes(join(appNodeModulesRoot, "@storyforge", "web")),
      webPackageStandaloneBytes: await sizePathBytes(join(appNodeModulesRoot, "@storyforge", "web", ".next", "standalone")),
    },
