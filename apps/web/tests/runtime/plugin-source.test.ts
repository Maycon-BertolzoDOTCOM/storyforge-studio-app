...
describe('derivePluginSourceLinks · github sources', () => {
  it('builds tree URL with ref + subpath when source has both', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'github',
        source:     'github:storyforge/plugins@v1.2.0/make-a-deck',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/storyforge/plugins/tree/v1.2.0/make-a-deck');
    expect(out.sourceLabel).toBe('storyforge/plugins @v1.2.0/make-a-deck');
    expect(out.sourceKindLabel).toBe('GitHub');
    expect(out.contributeUrl).toBe('https://github.com/storyforge/plugins/issues/new');
  ...
  it('falls back to repo-root URL when no ref / subpath', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'github',
        source:     'github:storyforge/plugins',
      }),
    );
    expect(out.sourceUrl).toBe('https://github.com/storyforge/plugins');
    expect(out.sourceLabel).toBe('storyforge/plugins');
  });
  ...
  it('uses pinnedRef when source has no inline ref', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/storyforge/plugins/tree/a1b2c3d4');
    expect(out.sourceLabel).toBe('storyforge/plugins @a1b2c3d4');
  });
  ...
  it('preserves slash-separated branch refs (release/1.0)', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/storyforge/plugins/tree/release/1.0');
  });
...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the StoryForge repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
    ...
    expect(out.sourceUrl).toBe('https://github.com/nexu-io/storyforge');
    expect(out.sourceKindLabel).toBe('Official');
    expect(out.sourceLabel).toBe('nexu-io/storyforge');
    expect(out.authorProfileUrl).toBe('https://github.com/nexu-io/storyforge');
    expect(out.homepageUrl).toBe('https://github.com/nexu-io/storyforge');
  });
});
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'StoryForge', url: 'https://github.com/nexu-io' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('StoryForge');
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        ...
        manifest: {
          name:    'p',
          version: '1.0.0',
          homepage: 'https://github.com/nexu-io/storyforge',
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.contributeUrl).toBe('https://github.com/nexu-io/storyforge/issues/new');
    expect(out.contributeOnGithub).toBe(true);
    expect(out.homepageUrl).toBe('https://github.com/nexu-io/storyforge');
  });
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('StoryForge')).toBe('OD');
