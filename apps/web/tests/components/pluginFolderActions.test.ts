...
// `nexu-io/storyforge`; `publish` drives `gh repo create / push` against the
...
describe('buildPluginFolderAgentActionPrompt', () => {
  ...
  describe('publish (repo-publish flow)', () => {
    ...
    it('bans the registry-submission CLI explicitly', () => {
      ...
      expect(prompt).toMatch(
        /Do NOT (call|route through) `?od plugin publish --to storyforge`?/i,
      );
      expect(prompt).toMatch(
        /registry[- ]submission|registry-submission flow|StoryForge PR/i,
      );
    });
  ...
  describe('contribute (PR-based flow)', () => {
    ...
    it('delegates StoryForge PR creation to the deterministic plugin CLI helper', () => {
      expect(prompt).toContain('nexu-io/storyforge');
      expect(prompt).toContain(`"$OD_NODE_BIN" "$OD_BIN" plugin storyforge-pr ${FOLDER}`);
    });
    ...
    it('states the CLI-owned PR workflow instead of re-listing shell steps', () => {
      ...
      // ban ("Do NOT call the legacy `od plugin publish --to storyforge`")
      // — verify the ban is in place, not the bare command.
      expect(prompt).toMatch(/do not call the legacy `od plugin publish --to storyforge`/i);
    });
    ...
    it('interpolates the actual folder path into manifest and copy steps', () => {
      ...
      expect(prompt).toContain(`plugin storyforge-pr ${FOLDER}`);
      expect(prompt).not.toContain('${folderPath}');
    });
