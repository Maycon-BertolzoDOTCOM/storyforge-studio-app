// Trigger prompt for the post-completion "Share to StoryForge" submission
...
export const SHARE_TO_COMMUNITY_PROMPT = [
  'Package the work I just finished in this project as an StoryForge plugin so I can install it into My plugins and open a PR to the StoryForge community catalog.',
  '',
  ...
  '',
  '**Do NOT** suggest follow-up CLI commands such as `od plugin publish`, `od plugin publish --to storyforge`, `gh repo create`, `git init` / `git remote add` / `git push`, or any other publish / repo wiring. The plugin-folder card under Design Files already exposes three buttons whose prompts drive those flows end-to-end with the right auth gates, fallbacks, and retry rules baked in:',
  ...
  '- **StoryForge PR** — opens a draft PR against `nexu-io/storyforge` for the community catalog.',
  '',
