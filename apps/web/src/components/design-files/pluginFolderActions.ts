...
export function buildPluginFolderAgentActionPrompt(
  ...
  return [
    ...
    `Manifest: \`${folderPath}/storyforge.json\``,
    '',
...
// `contribute` opens a draft PR against the `nexu-io/storyforge` community
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the StoryForge community catalog at `nexu-io/storyforge`.',
    ...
    `Manifest: \`${folderPath}/storyforge.json\``,
    '',
    'Run this deterministic StoryForge CLI workflow from the current project workspace:',
    '',
    `\`"$OD_NODE_BIN" "$OD_BIN" plugin storyforge-pr ${folderPath}\``,
    '',
    ...
    '- Do NOT call the legacy `od plugin publish --to storyforge` CLI — that flow produces an issue URL, which is the old path we are replacing.',
  ].join('\n');
}
...
// submission path — `od plugin publish --to storyforge` produces an
// StoryForge issue URL and belongs to the "StoryForge PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/storyforge.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to storyforge`. That command emits an StoryForge issue URL and belongs to the "StoryForge PR" button.',
    '',
    'Run this deterministic StoryForge CLI workflow from the current project workspace:',
    '',
    ...
    '',
    '**Hard constraints.** Treat these as inviolable:',
    '- Do NOT call `od plugin publish --to storyforge` (or any `--to <catalog>` variant). That is the registry-submission flow, not the repository-publish flow.',
