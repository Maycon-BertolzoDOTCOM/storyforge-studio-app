// Content fixtures for the "Use StoryForge everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'StoryForge works wherever your agent works',
    intro:
      'StoryForge is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires StoryForge as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without StoryForge at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start StoryForge in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
      'The daemon writes to `./.od/` (project-local) by default. Set ' +
      '`OD_DATA_DIR=~/.storyforge` to share data across projects.',
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive StoryForge from any shell',
    intro:
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          'corepack enable\n' +
          'pnpm install\n' +
          'pnpm --filter @storyforge-app/daemon build\n' +
          '\n' +
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose StoryForge as an MCP server to any coding agent',
    intro:
      'StoryForge ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover StoryForge tools ' +
    ...
    footer:
      'In the StoryForge app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    intro:
      ...
      'Server-Sent Events with the contract types in `@storyforge-app/contracts`.',
    bullets: [
    ...
    footer:
      ...
      '`@storyforge-app/contracts` — import them in your script for full ' +
      'autocomplete without wiring a generator.',
  },
  {
    id: 'skills',
    tabLabel: 'Skills & headless',
    heading: 'Drop-in Skills for any agent — even without StoryForge running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). StoryForge extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside StoryForge and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + StoryForge extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start StoryForge after ingesting.\n' +
          'pnpm tools-dev\n' +
