...
export const zhCN: Dict = {
  ...
  "chat.amrCard.switchBody":
    "可切换到 StoryForge 官方 AMR 模型服务，无需配置 API Key。完成登录、授权和充值后，将自动重试当前任务。",
  ...
  "chat.runError.signInMessage.amr":
    "StoryForge 智能体尚未登录，前往登录即可正常使用",
  "chat.runError.signInMessage.other":
    "{agent}尚未登录，请本地检查登录状态。推荐使用 StoryForge 智能体，更稳定划算",
  ...
  "chat.contextPrompt.referenceProject":
    "请把 StoryForge 项目“{name}”作为参考上下文。它的本地路径是：{path}。在设计或修改前，如有帮助请先搜索并读取这个目录。",
  ...
  "settings.welcomeTitle": "欢迎使用 StoryForge",
  "settings.welcomeSubtitle": "",
  "settings.onboardingCreateTitle": "从一句需求开始",
  "settings.onboardingCreateBody":
    "描述你想做的网站、应用、Deck、图片或视频，StoryForge 会创建项目，并保留可继续编辑的产物。",
  ...
  "settings.onboardingMemoryCalloutBody":
    "这些回答会构成你的记忆画像。StoryForge 会在每个任务中复用它，并在你工作时持续学习。",
  ...
  "settings.onboardingCloudTitle": "登录 StoryForge",
  "settings.onboardingCloudBody":
    "登录后可直接用云端 AI 开始设计，无需任何复杂配置。",
  "settings.onboardingCloudSignIn": "登录 StoryForge 云端",
  ...
  "settings.onboardingGateTooltipAmr":
    "后续流程由 AI 运行，请先登录 StoryForge AMR。",
  ...
  "settings.agentInstall.pathHint":
    "如果你通过 npm 或 Homebrew 安装了 CLI，但仍显示为未安装，请确认该工具的 bin 目录已加入 StoryForge daemon 继承的 PATH（在 macOS 上，Terminal 与 GUI 应用的 PATH 可能不同）。请参阅 QUICKSTART.md（“Local agent CLI and PATH” 章节）。",
  "settings.agentInstall.stepOpenLinks":
    "在目标代理卡片上打开“安装”或“文档”链接。",
  "settings.agentInstall.stepAuth":
    "返回 StoryForge 之前，请先在对应 CLI 中完成认证（登录或添加 API 凭据）。",
  ...
  "settings.mcpServerHint":
    "将 StoryForge 作为 MCP 服务器暴露给你的编码代理。",
  ...
  "settings.privacy": "隐私",
  "settings.privacyHint": "与 StoryForge 团队共享哪些数据",
  "settings.privacyConsentKicker": "帮助我们改进 StoryForge",
  "settings.privacyConsentLead":
    "StoryForge 可以将使用数据共享给我们的团队以协助改进。包括:",
  ...
  "settings.mcpHint":
    "让其他仓库中的编码助手（Claude Code、Cursor、VS Code、Antigravity、Zed、Windsurf）读取你的 StoryForge 项目。无需先导出 zip，即可将设计拉取到你的应用中。",
  "settings.mcpDaemonError":
    "无法连接到本地守护进程以解析安装路径（{error}）。请确保 StoryForge 正在运行，然后重新打开此面板。",
  "settings.mcpBuildDaemon": "请先构建守护进程。",
  "settings.mcpNodeMissing": "Node 二进制文件缺失。",
  "settings.mcpBuildHint":
    "apps/daemon/dist/cli.js 缺失。请运行 `pnpm --filter @storyforge-app/daemon build` 并刷新。",
  ...
  "settings.mcpCapabilityDefault":
    '默认使用你在 StoryForge 中打开的项目和文件，因此你可以直接说"在我的应用中构建这个"，无需重复说明是哪个设计。',
  "settings.mcpRunningNote":
    "StoryForge 必须处于运行状态，MCP 工具调用才能成功。如果你在打开 StoryForge 之前启动了编码助手，请重启助手以便它能连接到正在运行的守护进程。",
  ...
  "entry.followLinkedinLabel": "在 LinkedIn 上关注 StoryForge",
  "entry.followXiaohongshuLabel": "在小红书上关注 StoryForge",
  ...
  "handoff.promptIntro": "请基于这个 StoryForge 项目的本地文件夹继续实现：",
  ...
  "pluginsHome.subtitle":
    "当前运行环境内置的 StoryForge 工作流。选择一个加载 starter prompt，或浏览插件市场查看更多。",
  ...
  "pluginsHome.emptyCatalog":
    "目录为空。StoryForge 会随附内置插件，如果这里一直为空，请尝试重启 daemon。",
  ...
  "pluginCard.contributeAria": "将 {title} 贡献到 StoryForge",
  "pluginCard.contributeTitle": "通过 pull request 将插件贡献到 StoryForge",
  "skillPluginCandidate.createForMe": "创建插件/模板",
  "skillPluginCandidate.contributeToMain": "贡献到 storyforge",
  "skillPluginCandidate.repoDescription": "这个仓库看起来可以做成插件。",
  "integrations.kicker": "集成",
  "integrations.lede":
    "连接外部系统，把 MCP 工具带入智能体循环，并在其它 IDE、脚本和自动化中使用 StoryForge。",
  ...
  "mcpClient.daemonError":
    "无法连接本地 daemon。请确认 StoryForge 正在运行，然后重新打开此面板。",
  "mcpClient.saveFailed": "保存失败。请检查 daemon 是否正在运行后重试。",
  "useEverywhere.modalAria": "随处使用 StoryForge",
  "useEverywhere.modalTitle": "随处使用 StoryForge",
  "useEverywhere.modalSubtitle":
    "把 StoryForge 接入任何 IDE、智能体或脚本：CLI、HTTP、MCP 和 Skills。使用“复制智能体指南”，粘贴到 Claude Code、Codex、Cursor、openclaw 或 hermes 中即可完成配置。",
  ...
  "useEverywhere.section.overview.heading":
    "StoryForge 可在智能体工作的任何地方运行",
  "useEverywhere.section.overview.intro":
    "StoryForge 不只是一个窗口；它包含本地特权 daemon（`od`），以及 Skills + Design Systems + Atoms registry。它在你的机器上运行后，任何代码智能体（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes 或你自己的脚本）都可以通过四种可互换入口驱动生成、检查项目并产出设计制品。",
  "useEverywhere.section.overview.bullet1":
    "CLI — 使用 `od <command>` 支持无头脚本、CI 和 shell 自动化。",
  "useEverywhere.section.overview.bullet2":
    "MCP 服务器 — 将 StoryForge 作为 Model Context Protocol 服务器接入，使任何支持 MCP 的智能体都能列出技能、运行场景并读取制品。",
  ...
  "useEverywhere.section.overview.bullet4":
    "Skills — 可直接放入的 `SKILL.md` 包（兼容 Claude），任何已在 PATH 中的智能体都能调用，甚至不需要打开 StoryForge。",
  ...
  "useEverywhere.section.overview.footer":
    "daemon 默认写入项目本地的 `./.od/`。设置 `OD_DATA_DIR=~/.storyforge` 可在多个项目间共享数据。",
  "useEverywhere.section.cli.tab": "CLI · od",
  "useEverywhere.section.cli.heading": "从任意 shell 驱动 StoryForge",
  ...
  "useEverywhere.section.mcp.heading":
    "把 StoryForge 暴露为任意代码智能体可用的 MCP 服务器",
  "useEverywhere.section.mcp.intro":
    "StoryForge 内置 Model Context Protocol 服务器（`od mcp`），让任何支持 MCP 的客户端（Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes）都能发现 StoryForge 工具（列出技能、渲染预览、生成媒体、运行插件），无需手动 shell 调用。daemon 会通过 `GET /api/mcp/install-info` 为主流客户端发布可直接粘贴的安装片段。",
  ...
  "useEverywhere.section.mcp.footer":
    "在 StoryForge 应用中，打开“设置 → 集成”即可复制特定客户端的安装命令（Cursor、Claude Code、Antigravity、VS Code），无需手写 JSON。",
  ...
  "useEverywhere.section.http.intro":
    "本地 daemon 在 `http://127.0.0.1:7456` 提供 HTTP API（端口可配置）。Web UI 调用的每个端点也都可以供你的脚本使用。流式端点（聊天回合、项目运行）会发出 Server-Sent Events，事件类型定义在 `@storyforge-app/contracts` 中。",
  ...
  "useEverywhere.section.http.footer":
    "每个请求 / 响应的纯 TypeScript 类型都在 `@storyforge-app/contracts` 中；脚本中导入即可获得完整自动补全，无需接入生成器。",
  "useEverywhere.section.skills.tab": "Skills 和无头模式",
  "useEverywhere.section.skills.heading":
    "任意智能体可直接使用的 Skills，即使 StoryForge 未运行",
  "useEverywhere.section.skills.intro":
    "Skill 是一个包含 Claude 兼容 `SKILL.md`（YAML front matter + 正文）的目录。StoryForge 通过 `od:` 命名空间（`mode`、`preview`、`design_system`、`inputs` 等）扩展该格式，使同一个制品既能在 StoryForge 内使用，也能被 Claude Code、Codex、openclaw 或 hermes 这类普通智能体使用。发现逻辑遵循优先级链，因此项目可以覆盖自己的 skills。",
  ...
  "useEverywhere.section.skills.snippet1":
    "最小 SKILL.md（Claude 兼容 front matter + StoryForge 扩展）",
  ...
  "entry.githubStarAria": "在 GitHub 上为 StoryForge 点亮 Star",
  ...
  "newproj.locationDefault": "StoryForge 项目",
  ...
  "socialShare.openDesignText":
    "StoryForge 是开源的设计智能工作区，可以生成、编辑、部署并交付设计产物。",
  ...
  "socialShare.projectText":
    "我用 StoryForge 做了 {title}。查看已部署产物，也推荐收藏仓库：{repo}",
  ...
  "chat.brandBrowserAssistDesktopOnly":
    "浏览器辅助需要桌面应用，StoryForge 才能读取实时浏览器页面。",
  "chat.brandBrowserAssistReadFailed":
    "无法读取浏览器页面。请确认品牌网站已打开并完全加载。",
  "chat.brandBrowserAssistMessage":
    "StoryForge 需要浏览器辅助才能完成这次品牌提取。请先在 Browser 标签页清除页面拦截，然后在下方确认。",
  "brandEnrichment.title": "AI 优化",
  "brandEnrichment.body":
    "StoryForge 会在后台运行更深度的抽取，自动带上合适的设计系统 skills，并原地更新这个系统。通常需要 10-20 分钟，你可以继续做别的事。",
  ...
  "chat.activeFilePlaceholder": "让 StoryForge 修改 {file}...",
  ...
  "chat.mode.design.solves":
    "当你希望 StoryForge 生成文件、更新工作区，或把需求变成多模态可交付结果时使用。",
  ...
  "preview.errorBody":
    "示例 HTML 加载失败。请确认 StoryForge 正在运行后重试。",
  "preview.retry": "重试",
  ...
  "designFiles.documentTemplate.goalBody":
    "说明希望 StoryForge 基于这份文档生成、修改或判断什么。",
  ...
  "designFiles.documentTemplate.capabilityAgent":
    "StoryForge agent 可以读取、编辑这份文件，并把它作为项目上下文。",
  ...
  "designFiles.documentTemplate.nextBody":
    "检查并编辑这份文档，然后让 StoryForge 基于它生成、修改或执行操作。",
  ...
  "designFiles.usefulInfoTip18":
    "在 YouTube 上订阅 StoryForge，观看教程和演示。",
  "designFiles.usefulInfoTip19":
    "在 LinkedIn 上关注 StoryForge，了解产品动态。",
  "designFiles.usefulInfoTip20":
    "在小红书上关注 StoryForge，获取技巧和灵感。",
  ...
  "pet.navHint": "领养与自定义",
  "pet.tabBuiltIn": "内置",
  "pet.tabBuiltInHint": "StoryForge 内置的精选宠物 — 一键领养。",
  "pet.builtInEmpty":
  ...
  "pet.codexBundled": "内置",
  "pet.codexBundledTitle": "StoryForge 内置宠物，无需下载。",
  ...
  "settings.projectLocationsDescription":
    "添加可包含多个 StoryForge 项目文件夹的工作基目录。新项目会作为所选基目录内的一个文件夹保存。",
  ...
  "artifact.odCardBrandAssistBody":
    "在浏览器中打开品牌网站，让 StoryForge 继续提取此设计体系。",
  ...
  "updater.availableBody":
    "StoryForge {version} 可用。下载安装完成后即可打开安装器。",
  ...
  "updater.installerOpenBody":
    "安装器已打开。StoryForge 正在退出，以便你完成更新。",
  ...
  "updater.payloadReadyGeneric":
    "StoryForge 更新已就绪。StoryForge 会关闭并自动重启。",
  "updater.payloadReadyVersion":
    "StoryForge {version} 已就绪。StoryForge 会关闭并自动重启。",
  "updater.quitButton": "退出 StoryForge",
  "updater.quitFailedBody":
    "安装器已打开，但 StoryForge 无法退出。替换应用前请先退出 StoryForge。",
  "updater.quitFailedTitle": "无法退出",
  "updater.quitting": "正在退出…",
  "updater.ready": "更新已就绪",
  "updater.readyGeneric": "新版本已就绪。StoryForge 会关闭并打开安装器。",
  "updater.readyVersion":
    "StoryForge {version} 已就绪。StoryForge 会关闭并打开安装器。",
  ...
  'settings.updateQuitFailed': '安装程序已打开，但 StoryForge 未能自动退出。',
  ...
  "dsManager.createBody":
    "把你的品牌、产品、代码、素材和设计参考教给 StoryForge。",
  ...
  "settings.customInstructionsDesc":
    "StoryForge 在每次对话中都会遵循的固定指令。它们不是保存的记忆；事实、偏好和项目上下文请使用「记忆」。",
  ...
  "brand.librarySubtitle":
    "把你的设计系统教给 StoryForge——从网站提取一个设计系统，然后在任意对话中应用它。",
  ...
  "onboarding.buildBody":
    "StoryForge 把你的品牌沉淀为可复用的设计系统,让你之后构建的一切都保持品牌一致。",
  ...
  "dsCreate.sourceSectionBody":
    "从 GitHub 仓库、网站、DESIGN.md 或能体现风格的文件开始。StoryForge 会先快速创建一套可用系统，然后 AI 可在项目中继续细化。",
  ...
  "ds.extractionRunningBody": "StoryForge 正在就地更新此体系。",
  ...
  "ds.repoConnectBannerBody":
    "连接 GitHub 后，StoryForge 可以读取你的仓库，并把颜色、字体和组件提取到此设计体系中。",
