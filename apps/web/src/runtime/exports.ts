...
export { isOpenDesignHostAvailable } from '@storyforge-app/host';
...
export function buildDesignManifestContent(opts: {
  ...
  const title = opts.title || 'StoryForge artifact';
  ...
  return JSON.stringify({
    schema: 'storyforge.design-manifest.v1',
    title,
    kind: opts.kind ?? 'html',
    entryFile,
    sourceFiles: {
...
- Keep generated product UI free of StoryForge chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or StoryForge chrome in the production UI.
