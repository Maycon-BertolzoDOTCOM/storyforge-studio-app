...
import type { OpenDesignHostActionResult, OpenDesignHostCaptureResult, OpenDesignHostUpdaterActionOptions } from "@storyforge-app/host";
...
// sees the web's own "Loading StoryForge…" shell flash between the splash and
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  const windowTitle = options.windowTitle ?? "StoryForge";
  const window = new BrowserWindow({
    ...
    // web's own "Loading StoryForge…" shell.
    show: false,
    title: windowTitle,
    autoHideMenuBar: true,
    ...MAC_WINDOW_CHROME,
  ...
  window.webContents.on("did-start-loading", () => {
    console.info("[storyforge desktop] main window did-start-loading", {
      pendingUrl,
      url: window.webContents.getURL(),
    });
  });
  window.webContents.on("dom-ready", () => {
    console.info("[storyforge desktop] main window dom-ready", {
      title: window.getTitle(),
      url: window.webContents.getURL(),
    });
  });
  window.webContents.on("did-finish-load", () => {
    console.info("[storyforge desktop] main window did-finish-load", {
      title: window.getTitle(),
      url: window.webContents.getURL(),
    });
  });
  window.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error("[storyforge desktop] main window did-fail-load", {
      errorCode,
      errorDescription,
      isMainFrame,
      pendingUrl,
  ...
  window.on("unresponsive", () => {
    console.error("[storyforge desktop] main window unresponsive", {
      pendingUrl,
      url: window.webContents.getURL(),
    });
  });
  window.on("responsive", () => {
    console.info("[storyforge desktop] main window responsive", {
      pendingUrl,
      url: window.webContents.getURL(),
    });
  });
  ...
  window.webContents.on("render-process-gone", (_event, details) => {
    console.error("[storyforge desktop] main window render-process-gone", {
  ...
  const requireMainWindowSender = (event: Electron.IpcMainInvokeEvent): void => {
    if (event.sender !== window.webContents) {
      throw new Error("host IPC is only available to the main StoryForge window");
    }
  };
  ...
  // reveal the web's own dark "Loading StoryForge…" shell, and (b) the splash
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (url != null && (url !== currentUrl || rendererFailed)) {
        ...
        console.info("[storyforge desktop] main window loadURL start", { currentUrl, url });
        await window.loadURL(url);
        console.info("[storyforge desktop] main window loadURL success", { url });
        currentUrl = url;
  ...
  return {
    ...
    async eval(input) {
      ...
      const startedAt = Date.now();
      console.info("[storyforge desktop] eval executeJavaScript start", {
      ...
      try {
        ...
        console.info("[storyforge desktop] eval executeJavaScript success", {
        ...
        console.error("[storyforge desktop] eval executeJavaScript failed", {
          durationMs: Date.now() - startedAt,
