...
const STORY_EN: StoryCopy =
  {
    ...
    more: "Read more",
    quote: { lead: "StoryForge is our ", highlight: "unfair advantage." },
    name: "Anthony Reyes",
...
const STORY_ZH: StoryCopy =
  {
    ...
    more: "查看更多",
    quote: { lead: "StoryForge 是我们的", highlight: "不公平优势。" },
    name: "Anthony Reyes",
    role: "创始人，Ikigai One",
...
const STORY_ZH_TW: StoryCopy =
  {
    ...
    more: "查看更多",
    quote: { lead: "StoryForge 是我們的", highlight: "不公平優勢。" },
    name: "Anthony Reyes",
    role: "創辦人，Ikigai One",
...
const STORY_PT_BR: StoryCopy =
  {
    ...
    more: "Ler mais",
    quote: { lead: "StoryForge é nossa ", highlight: "vantagem injusta." },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const STORY_ES: StoryCopy =
  {
    ...
    more: "Leer más",
    quote: { lead: "StoryForge es nuestra ", highlight: "ventaja injusta." },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const STORY_RU: StoryCopy =
  {
    ...
    more: "Подробнее",
    quote: { lead: "StoryForge — наше ", highlight: "нечестное преимущество." },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const STORY_FR: StoryCopy =
  {
    ...
    more: "En savoir plus",
    quote: { lead: "StoryForge est notre ", highlight: "avantage déloyal." },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const STORY_KO: StoryCopy =
  {
    ...
    more: "자세히 보기",
    quote: { lead: "StoryForge은 우리의 ", highlight: "불공정한 우위입니다." },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const STORY_DE: StoryCopy =
  {
    ...
    more: "Mehr lesen",
    quote: { lead: "StoryForge ist unser ", highlight: "unfairer Vorteil." },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const STORY_JA: StoryCopy =
  {
    ...
    more: "詳しく見る",
    quote: { lead: "StoryForge は私たちの", highlight: "不公平な優位性。" },
    name: "Anthony Reyes",
    role: "Founder, Ikigai One",
...
const FAQ_EN: FaqItem[] =
  [
    { q: "What are StoryForge model credits? How are they used?", a: "StoryForge model credits are the USD-denominated balance used for StoryForge hosted models. Each hosted AI model call deducts from your StoryForge balance by usage. BYOK calls to provider models do not consume credits. Each subscription grants the matching credit amount every billing cycle." },
    { q: "How do the plans differ? Can Free use hosted models?", a: "Free does not include StoryForge model credits and supports local CLI plus BYOK only. Plus, Pro, and Max grant the model-credit amounts shown on their plan cards for different usage levels. Every plan, including Free, supports BYOK." },
    ...
    { q: "What happens when monthly credits run out? Do they roll over?", a: "StoryForge model credits are granted by billing cycle. They reset each cycle and unused credits do not roll over. When credits run out, you can upgrade, enable auto top-up, or use BYOK provider keys to keep calling models without spending StoryForge credits." },
    { q: "How does auto top-up work?", a: "After authorization, you set a monthly top-up cap. When the StoryForge balance is low, the system adds credits and charges your saved payment method. Total auto top-ups in a calendar month will not exceed your cap. You can update the cap or turn it off in Wallet at any time." },
    { q: "What is BYOK? Do all plans support it?", a: "BYOK means Bring Your Own Key: add your own Anthropic, OpenAI, Google, or other provider API key and call provider models directly. BYOK does not consume StoryForge balance and is available on every plan, including Free." },
    ...
    { q: "How do refunds work?", a: "If you'd like a refund, or run into a billing problem such as a duplicate charge or an unexpected charge, email support@storyforge.ai directly and we'll help review and sort it out. Canceling a subscription only stops billing for the next cycle — it does not automatically issue a refund." },
  ];

const FAQ_ZH: FaqItem[] =
  [
    { q: "StoryForge 模型额度是什么？怎么消耗？", a: "StoryForge 模型额度是用于调用 StoryForge 托管模型的余额，以美元计。每次调用 StoryForge 托管的 AI 模型时，按用量从你的 StoryForge 余额中扣减；使用 BYOK（自带 API Key）调用原厂模型不消耗额度。订阅后每个计费周期发放对应金额的额度。" },
    { q: "各套餐有什么区别？Free 能用托管模型吗？", a: "Free 不含 StoryForge 模型额度，仅支持本地 CLI 与 BYOK 自带密钥；Plus / Pro / Max 发放的 StoryForge 模型额度以套餐卡片当前显示为准，适用从个人轻度到团队高频的不同强度。所有套餐（含 Free）都支持 BYOK。" },
    ...
    { q: "月度额度用完了怎么办？会累积到下月吗？", a: "套餐的 StoryForge 模型额度按计费周期发放，每个周期重置、未用完不结转到下个周期。额度用完后可以：升级到更高套餐、开启自动充值在余额不足时自动补充，或改用 BYOK 自带密钥继续调用（不消耗额度）。" },
    { q: "自动充值是怎么工作的？", a: "授权后由你设定每月充值上限（这是唯一需要你设置的值）。当账户 StoryForge 余额不足时，系统会自动补充额度并从你绑定的支付方式扣款，单个自然月内的累计自动充值金额不会超过你设定的上限；触发时机与每次充值金额由系统自动管理。可随时在钱包中调整上限或关闭，关闭后不再自动扣款。" },
    { q: "BYOK 是什么？所有套餐都支持吗？", a: "BYOK（Bring Your Own Key）是指填入你自己的 Anthropic、OpenAI、Google 等厂商 API Key，直接调用原厂模型，不消耗 StoryForge 余额、也不受套餐模型列表限制。所有套餐（包括 Free）均支持 BYOK。" },
    ...
    { q: "怎么申请退款？", a: "如果你有退款诉求，或遇到重复扣款、异常扣费等账单问题，请直接联系 support@storyforge.ai，我们会协助核查处理。取消订阅本身只会停止下个账期扣费，不会自动发起退款。" },
  ];

const FAQ_ZH_TW: FaqItem[] =
  [
    { q: "StoryForge 模型額度是什麼？如何消耗？", a: "StoryForge 模型額度是用於調用 StoryForge 托管模型的美元餘額。每次調用托管 AI 模型時，會按用量從餘額扣減；使用 BYOK 調用原廠模型不消耗額度。訂閱後每個計費週期會發放對應額度。" },
    { q: "各套餐有什麼差異？Free 能使用托管模型嗎？", a: "Free 不含 StoryForge 模型額度，僅支援本地 CLI 與 BYOK。Plus、Pro、Max 發放的 StoryForge 模型額度以方案卡片目前顯示為準，適用不同使用強度。所有套餐（含 Free）都支援 BYOK。" },
    ...
    { q: "月度額度用完怎麼辦？會結轉嗎？", a: "StoryForge 模型額度按計費週期發放，每個週期重置，未用完不會結轉。額度用完後可以升級、開啟自動充值，或使用 BYOK 原廠密鑰繼續調用模型。" },
    { q: "自動充值如何運作？", a: "授權後你可以設定每月充值上限。當 StoryForge 餘額偏低時，系統會自動補充額度並向已綁定的支付方式扣款；單個自然月內自動充值總額不會超過上限。你可以隨時在錢包中調整或關閉。" },
    { q: "BYOK 是什麼？所有套餐都支援嗎？", a: "BYOK 指 Bring Your Own Key：填入自己的 Anthropic、OpenAI、Google 等廠商 API Key，直接調用原廠模型。BYOK 不消耗 StoryForge 餘額，所有套餐（含 Free）都支援。" },
    ...
    { q: "退款如何處理？", a: "如果你有退款需求，或遇到重複扣款、異常扣費等帳單問題，請直接聯絡 support@storyforge.ai，我們會協助核查處理。取消訂閱本身只會停止下個帳期扣費，不會自動發起退款。" },
  ];

const FAQ_PT_BR: FaqItem[] =
  [
    { q: "O que são créditos de modelo StoryForge? Como são usados?", a: "Créditos de modelo StoryForge são o saldo em USD usado para modelos hospedados pela StoryForge. Cada chamada de modelo hospedado desconta do saldo conforme o uso. Chamadas BYOK para modelos de provedores não consomem créditos. Cada assinatura concede créditos no ciclo de cobrança." },
    { q: "Como os planos diferem? O Free usa modelos hospedados?", a: "O Free não inclui créditos de modelo StoryForge e oferece apenas CLI local com BYOK. Plus, Pro e Max concedem os créditos exibidos nos cartões dos planos para diferentes níveis de uso. Todos os planos, incluindo Free, aceitam BYOK." },
    ...
    { q: "O que acontece quando os créditos acabam? Eles acumulam?", a: "Créditos são concedidos por ciclo de cobrança, reiniciam a cada ciclo e não acumulam. Ao acabar, você pode fazer upgrade, ativar recarga automática ou usar chaves BYOK para continuar chamando modelos sem gastar créditos StoryForge." },
    { q: "Como funciona a recarga automática?", a: "Após autorizar, você define um limite mensal. Quando o saldo StoryForge fica baixo, o sistema adiciona créditos e cobra o método de pagamento salvo. O total mensal não passa do limite. Você pode ajustar ou desligar no Wallet." },
    { q: "O que é BYOK? Todos os planos suportam?", a: "BYOK significa Bring Your Own Key: use sua chave Anthropic, OpenAI, Google ou de outro provedor para chamar modelos diretamente. BYOK não consome saldo StoryForge e está disponível em todos os planos." },
    ...
    { q: "Como funcionam reembolsos?", a: "Se você quiser um reembolso ou tiver algum problema de cobrança, como cobrança duplicada ou cobrança indevida, fale diretamente com support@storyforge.ai e vamos ajudar a verificar e resolver. Cancelar a assinatura apenas interrompe a cobrança do próximo ciclo — não gera reembolso automático." },
  ];

const FAQ_ES: FaqItem[] =
  [
    { q: "¿Qué son los créditos de modelo StoryForge? ¿Cómo se usan?", a: "Son el saldo en USD usado para modelos alojados por StoryForge. Cada llamada a un modelo alojado descuenta del saldo según el uso. Las llamadas BYOK a modelos de proveedor no consumen créditos. Cada suscripción concede créditos en cada ciclo de facturación." },
    { q: "¿En qué se diferencian los planes? ¿Free usa modelos alojados?", a: "Free no incluye créditos de modelo StoryForge y solo admite CLI local con BYOK. Plus, Pro y Max conceden los créditos mostrados en sus tarjetas para distintos niveles de uso. Todos los planes, incluido Free, admiten BYOK." },
    ...
    { q: "¿Qué pasa si se agotan los créditos? ¿Se acumulan?", a: "Los créditos se conceden por ciclo de facturación, se reinician en cada ciclo y no se acumulan. Al agotarse, puedes subir de plan, activar recarga automática o usar claves BYOK para seguir llamando modelos sin gastar créditos StoryForge." },
    { q: "¿Cómo funciona la recarga automática?", a: "Tras autorizarla, defines un límite mensual. Cuando el saldo StoryForge baja, el sistema añade créditos y cobra el método de pago guardado. El total mensual no supera tu límite. Puedes cambiarlo o desactivarlo en Wallet." },
    { q: "¿Qué es BYOK? ¿Todos los planes lo admiten?", a: "BYOK significa Bring Your Own Key: usa tu propia clave de Anthropic, OpenAI, Google u otro proveedor para llamar modelos directamente. BYOK no consume saldo StoryForge y está disponible en todos los planes." },
    ...
    { q: "¿Cómo funcionan los reembolsos?", a: "Si quieres un reembolso o tienes algún problema de facturación, como un cargo duplicado o un cargo indebido, escribe directamente a support@storyforge.ai y te ayudaremos a revisarlo y resolverlo. Cancelar la suscripción solo detiene el cobro del próximo ciclo; no genera un reembolso automático." },
  ];

const FAQ_RU: FaqItem[] =
  [
    { q: "Что такое кредиты моделей StoryForge и как они расходуются?", a: "Кредиты моделей StoryForge — это баланс в USD для моделей, размещенных StoryForge. Каждый вызов размещенной модели списывает баланс по фактическому использованию. Вызовы BYOK к моделям провайдеров кредиты не тратят. Каждая подписка выдает кредиты в каждом расчетном цикле." },
    { q: "Чем отличаются планы? Может ли Free использовать размещенные модели?", a: "Free не включает кредиты моделей StoryForge и поддерживает только локальный CLI с BYOK. Plus, Pro и Max дают кредиты, показанные на карточках планов, для разных уровней использования. Все планы, включая Free, поддерживают BYOK." },
    ...
    { q: "Что будет, когда месячные кредиты закончатся? Они переносятся?", a: "Кредиты выдаются по расчетным циклам, сбрасываются каждый цикл и не переносятся. Когда они заканчиваются, можно повысить план, включить автопополнение или использовать ключи BYOK для вызова моделей без расхода кредитов StoryForge." },
    { q: "Как работает автопополнение?", a: "После авторизации вы задаете месячный лимит. Когда баланс StoryForge становится низким, система добавляет кредиты и списывает оплату с сохраненного метода. Сумма автопополнений за месяц не превысит лимит. Лимит можно изменить или отключить в Wallet." },
    { q: "Что такое BYOK? Все планы это поддерживают?", a: "BYOK означает Bring Your Own Key: добавьте собственный ключ Anthropic, OpenAI, Google или другого провайдера и вызывайте модели напрямую. BYOK не расходует баланс StoryForge и доступен во всех планах." },
    ...
    { q: "Как работают возвраты?", a: "Если вы хотите вернуть средства или столкнулись с проблемой оплаты — двойным списанием или ошибочным списанием, — напишите напрямую на support@storyforge.ai, и мы поможем всё проверить и решить. Отмена подписки лишь останавливает списание за следующий период и не создаёт возврат автоматически." },
  ];

const FAQ_FR: FaqItem[] =
  [
    { q: "Que sont les crédits de modèles StoryForge ? Comment sont-ils utilisés ?", a: "Ce sont des crédits en USD utilisés pour les modèles hébergés par StoryForge. Chaque appel à un modèle hébergé débite le solde selon l'usage. Les appels BYOK vers des modèles fournisseurs ne consomment pas de crédits. Chaque abonnement accorde les crédits correspondants à chaque cycle." },
    { q: "Quelle est la différence entre les plans ? Free utilise-t-il les modèles hébergés ?", a: "Free n'inclut pas de crédits StoryForge et prend seulement en charge le CLI local avec BYOK. Plus, Pro et Max accordent les crédits affichés sur leurs cartes pour différents niveaux d'usage. Tous les plans, y compris Free, prennent en charge BYOK." },
    ...
    { q: "Que se passe-t-il quand les crédits mensuels sont épuisés ? Sont-ils reportés ?", a: "Les crédits sont accordés par cycle de facturation, réinitialisés à chaque cycle et non reportés. Une fois épuisés, vous pouvez passer à un plan supérieur, activer la recharge automatique ou utiliser BYOK pour continuer sans consommer de crédits StoryForge." },
    { q: "Comment fonctionne la recharge automatique ?", a: "Après autorisation, vous définissez un plafond mensuel. Quand le solde StoryForge est bas, le système ajoute des crédits et facture le moyen de paiement enregistré. Le total mensuel ne dépasse pas le plafond. Vous pouvez l'ajuster ou la désactiver dans Wallet." },
    { q: "Qu'est-ce que BYOK ? Tous les plans le prennent-ils en charge ?", a: "BYOK signifie Bring Your Own Key : ajoutez votre propre clé Anthropic, OpenAI, Google ou autre fournisseur pour appeler les modèles directement. BYOK ne consomme pas le solde StoryForge et est disponible sur tous les plans." },
    ...
    { q: "Comment fonctionnent les remboursements ?", a: "Si vous souhaitez un remboursement ou rencontrez un problème de facturation, comme un double débit ou un débit anormal, écrivez directement à support@storyforge.ai et nous vous aiderons à vérifier et à régler cela. Annuler l'abonnement arrête seulement le prélèvement du prochain cycle ; cela ne déclenche pas de remboursement automatique." },
  ];

const FAQ_KO: FaqItem[] =
  [
    { q: "StoryForge 모델 크레딧은 무엇이며 어떻게 사용되나요?", a: "StoryForge 모델 크레딧은 StoryForge 호스팅 모델에 사용하는 USD 기준 잔액입니다. 호스팅 AI 모델을 호출할 때 사용량에 따라 잔액에서 차감됩니다. BYOK로 제공자 모델을 호출하는 경우 크레딧을 사용하지 않습니다. 각 구독은 결제 주기마다 해당 크레딧을 지급합니다." },
    { q: "각 플랜은 어떻게 다른가요? Free도 호스팅 모델을 쓸 수 있나요?", a: "Free에는 StoryForge 모델 크레딧이 포함되지 않으며 로컬 CLI와 BYOK만 지원합니다. Plus, Pro, Max는 플랜 카드에 표시된 크레딧을 제공해 사용량 수준에 맞게 선택할 수 있습니다. Free를 포함한 모든 플랜은 BYOK를 지원합니다." },
    ...
    { q: "월간 크레딧을 모두 쓰면 어떻게 되나요? 이월되나요?", a: "크레딧은 결제 주기마다 지급되고 주기마다 초기화되며 이월되지 않습니다. 크레딧이 부족하면 상위 플랜으로 업그레이드하거나 자동 충전을 켜거나 BYOK 키로 StoryForge 크레딧 없이 모델을 계속 호출할 수 있습니다." },
    { q: "자동 충전은 어떻게 작동하나요?", a: "승인 후 월간 충전 한도를 설정합니다. StoryForge 잔액이 낮아지면 시스템이 크레딧을 추가하고 저장된 결제 수단으로 청구합니다. 한 달 자동 충전 총액은 한도를 넘지 않습니다. Wallet에서 한도를 조정하거나 끌 수 있습니다." },
    { q: "BYOK란 무엇인가요? 모든 플랜에서 지원하나요?", a: "BYOK는 Bring Your Own Key의 약자로, Anthropic, OpenAI, Google 등 제공자의 API 키를 직접 추가해 모델을 호출하는 방식입니다. BYOK는 StoryForge 잔액을 사용하지 않으며 모든 플랜에서 지원됩니다." },
    ...
    { q: "환불은 어떻게 처리되나요?", a: "환불을 원하거나 중복 청구, 비정상 청구 같은 결제 문제가 있으면 support@storyforge.ai로 직접 문의해 주세요. 저희가 확인하고 처리해 드립니다. 구독 취소는 다음 결제 주기의 청구만 중단하며, 자동으로 환불을 진행하지는 않습니다." },
  ];

const FAQ_DE: FaqItem[] =
  [
    { q: "Was sind StoryForge Modell-Credits und wie werden sie genutzt?", a: "StoryForge Modell-Credits sind ein USD-Guthaben für von StoryForge gehostete Modelle. Jeder Aufruf eines gehosteten Modells belastet das Guthaben nach Nutzung. BYOK-Aufrufe zu Anbietermodellen verbrauchen keine Credits. Jede Subscription gewährt Credits pro Abrechnungszyklus." },
    { q: "Wie unterscheiden sich die Pläne? Kann Free gehostete Modelle nutzen?", a: "Free enthält keine StoryForge Modell-Credits und unterstützt nur lokale CLI plus BYOK. Plus, Pro und Max gewähren die auf ihren Plankarten angezeigten Credits für unterschiedliche Nutzungslevel. Alle Pläne, einschließlich Free, unterstützen BYOK." },
    ...
    { q: "Was passiert, wenn monatliche Credits aufgebraucht sind? Werden sie übertragen?", a: "Credits werden pro Abrechnungszyklus gewährt, in jedem Zyklus zurückgesetzt und nicht übertragen. Wenn sie aufgebraucht sind, kannst du upgraden, Auto-Top-up aktivieren oder BYOK-Schlüssel nutzen, um Modelle ohne StoryForge Credits aufzurufen." },
    { q: "Wie funktioniert Auto-Top-up?", a: "Nach der Autorisierung legst du ein Monatslimit fest. Wenn das StoryForge Guthaben niedrig ist, fügt das System Credits hinzu und belastet die gespeicherte Zahlungsmethode. Die Summe im Kalendermonat überschreitet das Limit nicht. Du kannst es in Wallet ändern oder deaktivieren." },
    { q: "Was ist BYOK? Unterstützen alle Pläne das?", a: "BYOK bedeutet Bring Your Own Key: Du fügst deinen eigenen API-Schlüssel von Anthropic, OpenAI, Google oder anderen Anbietern hinzu und rufst Modelle direkt auf. BYOK verbraucht kein StoryForge Guthaben und ist in allen Plänen verfügbar." },
    ...
    { q: "Wie funktionieren Rückerstattungen?", a: "Wenn du eine Rückerstattung möchtest oder ein Abrechnungsproblem hast, etwa eine doppelte oder fehlerhafte Abbuchung, schreib direkt an support@storyforge.ai – wir helfen dir, das zu prüfen und zu klären. Eine Kündigung stoppt nur die Abbuchung für den nächsten Abrechnungszeitraum und löst keine automatische Rückerstattung aus." },
  ];

const FAQ_JA: FaqItem[] =
  [
    { q: "StoryForge モデルクレジットとは何ですか？どのように消費されますか？", a: "StoryForge モデルクレジットは、StoryForge がホストするモデルに使う USD 建ての残高です。ホストモデルを呼び出すたびに使用量に応じて差し引かれます。BYOK でプロバイダーモデルを呼び出す場合、クレジットは消費されません。各サブスクリプションは請求サイクルごとに対応するクレジットを付与します。" },
    { q: "各プランの違いは？Free でホストモデルは使えますか？", a: "Free には StoryForge モデルクレジットが含まれず、ローカル CLI と BYOK のみをサポートします。Plus、Pro、Max はプランカードに表示されるクレジットを付与し、利用量に応じて選べます。Free を含むすべてのプランで BYOK を利用できます。" },
    ...
    { q: "月間クレジットを使い切るとどうなりますか？繰り越されますか？", a: "クレジットは請求サイクルごとに付与され、各サイクルでリセットされます。未使用分は繰り越されません。使い切った場合はアップグレード、自動チャージの有効化、または BYOK キーを使って StoryForge クレジットを使わずにモデル呼び出しを継続できます。" },
    { q: "自動チャージはどのように機能しますか？", a: "承認後、月間チャージ上限を設定します。StoryForge 残高が少なくなると、システムがクレジットを追加し、保存済みの支払い方法に請求します。月間合計は上限を超えません。Wallet で上限変更や停止ができます。" },
    { q: "BYOK とは何ですか？すべてのプランで使えますか？", a: "BYOK は Bring Your Own Key の略で、自分の Anthropic、OpenAI、Google などの API キーを追加してモデルを直接呼び出す方式です。BYOK は StoryForge 残高を消費せず、すべてのプランで利用できます。" },
    ...
    { q: "返金はどのように扱われますか？", a: "返金をご希望の場合や、重複請求・不正請求などの請求に関する問題が発生した場合は、support@storyforge.ai まで直接ご連絡ください。確認のうえ対応いたします。サブスクリプションのキャンセルは次の請求期間の課金を停止するだけで、自動的に返金は行われません。" },
  ];
...
export const ENTERPRISE_EMAIL = 'support@storyforge.ai';
...
const ENTERPRISE_ZH: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "提交失败，请稍后重试，或邮件联系 support@storyforge.ai",
    close: "关闭",
  },
};
...
const ENTERPRISE_EN: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Couldn't submit — please try again or email support@storyforge.ai",
    close: "Close",
  },
};
...
const ENTERPRISE_ZH_TW: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "提交失敗，請稍後重試，或來信 support@storyforge.ai",
    close: "關閉",
  },
};
...
const ENTERPRISE_PT_BR: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Não foi possível enviar. Tente novamente ou escreva para support@storyforge.ai",
    close: "Fechar",
  },
};
...
const ENTERPRISE_ES: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "No se pudo enviar. Inténtalo de nuevo o escribe a support@storyforge.ai",
    close: "Cerrar",
  },
};
...
const ENTERPRISE_RU: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Не удалось отправить. Повторите попытку или напишите на support@storyforge.ai",
    close: "Закрыть",
  },
};
...
const ENTERPRISE_FR: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Échec de l'envoi — réessayez ou écrivez à support@storyforge.ai",
    close: "Fermer",
  },
};
...
const ENTERPRISE_KO: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "제출에 실패했습니다. 다시 시도하거나 support@storyforge.ai로 문의해 주세요.",
    close: "닫기",
  },
};
...
const ENTERPRISE_DE: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Senden fehlgeschlagen — bitte erneut versuchen oder an support@storyforge.ai schreiben",
    close: "Schließen",
  },
};
...
const ENTERPRISE_JA: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "送信に失敗しました。もう一度お試しいただくか、support@storyforge.ai までご連絡ください。",
    close: "閉じる",
  },
};
