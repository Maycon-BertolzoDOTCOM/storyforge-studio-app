...
const RAW_REPO = 'https://raw.githubusercontent.com/nexu-io/storyforge/main';
const findRepoRoot = () => {
  const candidates = [
    process.cwd(),
...
const registryUrlFor = (registryId: string) =>
  `${RAW_REPO}/plugins/registry/${registryId}/storyforge-marketplace.json`;
...
const findManifestFiles = (dir: string): string[] => {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (entry.isFile() && entry.name === 'storyforge.json') {
...
const entryFromMarketplace = (
  ...
  const rawDescription =
    explicitLocalizedString(
      ...
      locale,
    ) ?? 'Agent-native StoryForge workflow packaged as a portable plugin.';
...
const loadRegistryEntries = (
  ...
  for (const dirent of readdirSync(REGISTRY_ROOT, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(REGISTRY_ROOT, registryId, 'storyforge-marketplace.json');
...
const officialEntryFromManifest = (
  ...
  const id = `storyforge/${pluginName}`;
  const pluginDir = path.dirname(manifestPath);
  const repoPath = toPosix(path.relative(REPO_ROOT, pluginDir));
  const source = `github:nexu-io/storyforge@main/${repoPath}`;
  ...
  const rawDescription =
    explicitLocalizedString(
      ...
      locale,
    ) ?? 'First-party StoryForge workflow packaged as a portable plugin.';
  const localized = localizePluginText({
    id,
    title: rawTitle,
    description: rawDescription,
