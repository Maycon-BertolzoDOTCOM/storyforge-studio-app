...
test('install.sh delegates to the StoryForge CLI installer with the requested agent', () => {
...
if [ "$1" = "mcp" ] && [ "$2" = "install" ] && [ "$3" = "--storyforge-cli-probe" ]; then
  printf '%s\\n' 'storyforge-cli:mcp-install:v1'
  exit 0
fi
printf '%s\\n' "$@" > "${argvOut}"
`,
...
if [ "$3" = "--storyforge-cli-probe" ]; then
  printf '%s\\n' 'Usage: od [OPTION]... [FILE]...'
  exit 0
fi
printf '%s\\n' "$@" > "${argvOut}"
...
`,
  ...
  try {
    ...
    assert.match(result.stderr, /does not look like the StoryForge CLI/);
