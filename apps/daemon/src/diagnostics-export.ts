...
import {
  ...
  diagnosticsFileName,
  type LogSource,
} from '@storyforge-app/diagnostics';
import {
  APP_KEYS,
...
export interface DiagnosticsHandlerOptions {
  ...
  runsDir?: string | null;
  /** StoryForge data dir (OD_DATA_DIR), used to locate the AMR OpenCode home. */
  dataDir?: string | null;
}
  ...
  return async (_req, res) => {
    try {
      ...
      const result = await buildDiagnosticsZip({
        ...
        redaction: { username },
        crashReports: {
          // Restrict to StoryForge's own process names. A generic "Electron"
          ...
          matchSubstrings: ['StoryForge', 'storyforge'],
          withinDays: 7,
          maxReports: 10,
          homeDir: home,
        },
