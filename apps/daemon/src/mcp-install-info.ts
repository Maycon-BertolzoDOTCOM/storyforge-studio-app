...
// and free of @storyforge-app/sidecar-proto so it can be unit-tested
...
export interface BuildMcpInstallPayloadInputs {
  ...
  /** Browser-facing StoryForge studio base URL (e.g.
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `StoryForge CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall StoryForge or Node and restart the daemon.`,
    );
  }
