...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  function computeInstallPayload(): McpInstallPayload {
    ...
    // knows where the browser-facing StoryForge studio is running.
  ...
  const CODEX_MCP_NAME = 'storyforge';
  ...
  // External MCP server configuration. StoryForge connects to these as a
