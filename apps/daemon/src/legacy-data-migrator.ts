 ...
 *
 * StoryForge 0.3.x ran from the repo and wrote runtime state to
 ...
 * way to know about it. See https://github.com/nexu-io/storyforge/issues/710.
 *
...
export function migrateLegacyDataDirSync(
  ...
  if (!legacyDirHasPayload(legacyDir)) {
    ...
    throw new LegacyMigrationError(
      'legacy_dir_invalid',
      `OD_LEGACY_DATA_DIR="${legacyDir}" is not a usable legacy data dir (expected app.sqlite directly inside it). Quit StoryForge, fix the path, and relaunch.`,
    );
  }
