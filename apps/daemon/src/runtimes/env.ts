...
import { mergeProxyAwareEnv, resolveSystemProxyEnv } from '@storyforge-app/platform';
...
//
// 1. Provider BYOK is separate. It is used by StoryForge's direct provider
...
export function spawnEnvForAgent(
  ...
  if (agentId === 'amr') {
    ...
    // Identify StoryForge as the host so the vela CLI tags its command +
