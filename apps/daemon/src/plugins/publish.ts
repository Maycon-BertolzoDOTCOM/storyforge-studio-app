...
//   - storyforge            → nexu-io/storyforge (plugins/community/<plugin-name>/).
  ...
  switch (args.catalog) {
    ...
    case 'storyforge': {
      const bodyWithRegistry = [
        body,
        '',
        '## StoryForge registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/storyforge.json`',
        '- Generated index: `plugins/registry/community/storyforge-marketplace.json`',
        '- Required checks: `od plugin validate`, `od plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
      const url = newIssueUrl('nexu-io/storyforge', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'nexu-io/storyforge',
        url,
...
export function upsertMarketplaceJsonEntry(args: {
  ...
  const manifest: MarketplaceJsonManifest = {
    ...
    name: typeof existing.name === 'string' ? existing.name : 'storyforge-marketplace',
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `storyforge.json` sidecar (additive StoryForge metadata).');
  lines.push('');
  lines.push('Generated by `od plugin publish` — see https://storyforge.ai/docs/plugins-spec.md.');
  return lines.join('\n');
}
