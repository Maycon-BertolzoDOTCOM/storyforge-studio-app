...
// + storyforge.json pair) and register every match into the
...
import type { InstalledPluginRecord, MarketplaceTrust } from '@storyforge-app/contracts';
...
export async function registerBundledPlugins(
  ...
  for (const tier of topLevel) {
    ...
    const tierManifest = path.join(tierAbs, 'storyforge.json');
    if (await pathExists(tierManifest)) {
      // Direct: <bundledRoot>/<plugin-id>/storyforge.json
    ...
    for (const entry of inner) {
      ...
      const manifest = path.join(folder, 'storyforge.json');
