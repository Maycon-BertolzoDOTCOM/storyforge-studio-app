...
// validation lives in `@storyforge-app/plugin-runtime`'s parser and we only
...
} from '@storyforge-app/plugin-runtime';
import {
...
const DEFAULT_MARKETPLACE_REPO = 'nexu-io/storyforge';
const DEFAULT_MARKETPLACE_REPO_REF = 'main';
const DEFAULT_MARKETPLACE_REGISTRY_PATH = 'plugins/registry';
const PUBLIC_MARKETPLACE_BASE_URL = 'https://storyforge.ai/marketplace';
const PUBLIC_PLUGINS_BASE_URL = 'https://storyforge.ai/plugins';
...
export function marketplaceManifestUrlForRegistry(id: string): string {
  ...
  return `${marketplaceRegistryBaseUrl()}/${registryId}/storyforge-marketplace.json`;
}

function registryIdFromBaseUrl(url: string, baseUrl: string): string | null {
  const base = baseUrl.replace(/\/+$/, '');
  if (!url.startsWith(`${base}/`) || !url.endsWith('/storyforge-marketplace.json')) {
    return null;
  }
  const id = url
    .slice(base.length + 1)
    .replace(/\/storyforge-marketplace\.json$/, '');
...
export function marketplaceRegistryIdFromUrl(url: string): string | null {
  ...
  for (const base of publicBases) {
    if (trimmed === `${base}/storyforge-marketplace.json`) return 'official';
    if (trimmed.startsWith(`${base}/`) && trimmed.endsWith('/storyforge-marketplace.json')) {
      const id = trimmed
        .slice(base.length + 1)
        .replace(/\/storyforge-marketplace\.json$/, '');
  ...
  try {
    ...
    return id && filename === 'storyforge-marketplace.json' ? id : null;
  } catch {
    return null;
  }
}
