...
//   - storyforge.json (required; this is what the installer
//     resolves first)
...
export interface PackPluginInput {
  // Path to the plugin folder. Must contain storyforge.json.
  folder: string;
...
export async function packPlugin(input: PackPluginInput): Promise<PackPluginResult> {
  ...
  // Confirm the folder shape — storyforge.json must exist + parse.
  let manifestRaw: string;
  try {
    manifestRaw = await fsp.readFile(path.join(folder, 'storyforge.json'), 'utf8');
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new PackPluginError(`folder ${folder} does not contain storyforge.json`);
    }
    throw new PackPluginError(`failed to read storyforge.json: ${(err as Error).message}`);
  }
  ...
  try {
    ...
    throw new PackPluginError(`storyforge.json failed to parse as JSON: ${(err as Error).message}`);
  }
