...
// frontmatter the skills protocol expects) + storyforge.json (sidecar
...
export async function scaffoldPlugin(input: ScaffoldInput): Promise<ScaffoldResult> {
  ...
  try {
    ...
    const conflicts = entries.filter((e) =>
      e === 'SKILL.md' || e === 'storyforge.json' || e === '.claude-plugin' || e === 'README.md',
    );
  ...
  const manifestPath = path.join(folder, 'storyforge.json');
  ...
  const readme = [
    ...
    '- `storyforge.json` — the versioned StoryForge marketplace sidecar.',
    '',
    'Edit `SKILL.md` to teach the agent how to perform the workflow.',
    'Edit `storyforge.json` to refine the marketplace card and inputs.',
    '',
  ].join('\n');
