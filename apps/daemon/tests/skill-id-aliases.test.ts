...
// Regression coverage for the editorial-collage → storyforge-landing rename.
...
beforeAll(async () => {
  ...
  await mkdir(path.join(skillsRoot, 'storyforge-landing'), { recursive: true });
  await writeFile(
    path.join(skillsRoot, 'storyforge-landing', 'SKILL.md'),
    '---\nname: storyforge-landing\ndescription: Atelier Zero landing.\n---\n\nbody\n',
    'utf8',
  );
  await mkdir(path.join(skillsRoot, 'storyforge-landing-deck'), {
    recursive: true,
  });
  await writeFile(
    path.join(skillsRoot, 'storyforge-landing-deck', 'SKILL.md'),
    '---\nname: storyforge-landing-deck\ndescription: Atelier Zero deck.\n---\n\nbody\n',
    'utf8',
  );
...
describe('SKILL_ID_ALIASES', () => {
  it('maps the editorial-collage rename to its current canonical id', () => {
    expect(SKILL_ID_ALIASES['editorial-collage']).toBe('storyforge-landing');
...
describe('resolveSkillId', () => {
  it('forwards deprecated ids to their canonical replacement', () => {
    expect(resolveSkillId('editorial-collage')).toBe('storyforge-landing');
...
describe('findSkillById', () => {
  it('resolves a project saved with the old editorial-collage id to the renamed skill', async () => {
    ...
    expect(skill.id).toBe('storyforge-landing');
  ...
  it('resolves a project saved with the old editorial-collage-deck id to the renamed deck skill', async () => {
    ...
    expect(skill.id).toBe('storyforge-landing-deck');
  });

  it('still resolves current ids exactly', async () => {
    const skills = await listSkills(skillsRoot);
    expect(findSkillById(skills, 'storyforge-landing')?.id).toBe(
      'storyforge-landing',
    );
  ...
  it('returns undefined for unknown ids and missing inputs', async () => {
    ...
    expect(findSkillById(null, 'storyforge-landing')).toBeUndefined();
  });
});
