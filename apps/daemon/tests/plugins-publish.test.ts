...
const META = {
  pluginId:          'storyforge/sample-plugin',
  pluginVersion:     '1.0.0',
  pluginTitle:       'Sample Plugin',
  pluginDescription: 'A fixture for the publish flow.',
  repoUrl:           'https://github.com/storyforge/sample-plugin',
};
...
describe('buildPublishLink', () => {
  ...
  it('builds a github-issue URL for anthropics/skills with title + body', () => {
    ...
    expect(link.prBody).toContain('https://github.com/storyforge/sample-plugin');
  });
  ...
  it('points at skills.sh + the npx skills add command (no PR form there)', () => {
    ...
    expect(link.prBody).toContain('npx skills add storyforge/sample-plugin');
  });

  it('builds an StoryForge registry submission URL', () => {
    // The dedicated `storyforge/plugin-registry` repo per
    // docs/plans/plugin-registry.md §1.2 is the long-term target; until that
    // operational launch step happens, submissions land in `nexu-io/storyforge`
    // (plugins/community/<plugin-name>/), keeping contribution where stars and
    // PR traffic already are.
    const link = buildPublishLink({ catalog: 'storyforge', meta: META });
    expect(link.catalogLabel).toBe('nexu-io/storyforge');
    expect(link.url).toMatch(/^https:\/\/github\.com\/nexu-io\/storyforge\/issues\/new\?/);
    expect(link.prBody).toContain('plugins/community/<plugin-name>/storyforge.json');
    expect(link.prBody).toContain('plugins/registry/community/storyforge-marketplace.json');
  });
...
describe('upsertMarketplaceJsonEntry', () => {
  it('adds a namespaced plugin entry with a reproducible github source', () => {
    ...
    expect(outcome.entry).toMatchObject({
      name: 'storyforge/sample-plugin',
      source: 'github:storyforge/sample-plugin',
      version: '1.0.0',
      title: 'Sample Plugin',
      publisher: {
        github: 'storyforge',
  ...
  it('updates existing entries and preserves unrelated catalog metadata', () => {
    const outcome = upsertMarketplaceJsonEntry({
      ...
      manifest: {
        ...
        plugins: [
          {
            name: 'storyforge/sample-plugin',
            source: 'github:storyforge/sample-plugin@old',
            version: '0.9.0',
            tags: ['kept'],
          },
        ],
      ...
      meta: {
        ...META,
        pluginVersion: '1.1.0',
        repoUrl: 'https://github.com/storyforge/sample-plugin/tree/main/plugins/sample',
      },
    });
    ...
    expect(outcome.manifest.plugins[0]).toMatchObject({
      name: 'storyforge/sample-plugin',
      source: 'github:storyforge/sample-plugin@main/plugins/sample',
      version: '1.1.0',
      tags: ['kept'],
    });
  });
