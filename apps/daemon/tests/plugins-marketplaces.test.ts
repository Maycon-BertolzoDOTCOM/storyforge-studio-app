...
const VALID_MANIFEST = JSON.stringify({
  ...
  plugins: [
    { name: 'sample-plugin', source: 'github:storyforge/sample-plugin', version: '0.1.0' },
  ],
});
...
describe('marketplaces', () => {
  ...
  it('normalizes public marketplace urls to the canonical raw registry', async () => {
    ...
    const result = await addMarketplace(db, {
      url: 'https://storyforge.ai/marketplace/community/storyforge-marketplace.json',
      fetcher: async (url) => {
        seenUrls.push(url);
        return {
          ok: true,
  ...
  it('normalizes legacy branch raw urls to the canonical raw registry', () => {
    expect(resolveMarketplaceFetchUrl(
      'https://raw.githubusercontent.com/nexu-io/storyforge/garnet-hemisphere/plugins/registry/community/storyforge-marketplace.json',
    )).toBe(marketplaceManifestUrlForRegistry('community'));
  });

  it('requires a raw storyforge-marketplace.json document, not a GitHub tree page', async () => {
    const result = await addMarketplace(db, {
      url: 'https://github.com/nexu-io/storyforge/tree/garnet-hemisphere/plugins/registry/community',
  ...
  it('refresh re-fetches and updates refreshed_at', async () => {
    ...
    updatedManifest.plugins.push({
      name: 'new-plugin',
      source: 'github:storyforge/new-plugin',
      version: '0.2.0',
    });
  ...
  it('seeds the checked-in default community registry as restricted and resolvable', async () => {
    const communityManifest = await readFile(
      new URL('../../../plugins/registry/community/storyforge-marketplace.json', import.meta.url),
      'utf8',
    );
    ...
    expect(resolved?.source).toMatch(
      /^github:nexu-io\/storyforge(?:@[^/]+)?\/plugins\/community\/registry-starter$/,
    );
  });

  it('keeps the checked-in official registry populated from bundled plugins', async () => {
    const officialManifestText = await readFile(
      new URL('../../../plugins/registry/official/storyforge-marketplace.json', import.meta.url),
      'utf8',
    );
    ...
    expect(officialManifest.plugins?.some((plugin) => plugin.name === 'storyforge/build-test')).toBe(true);
    expect(officialManifest.plugins?.every((plugin) =>
      /^github:nexu-io\/storyforge(?:@[^/]+)?\/plugins\/_official\//.test(plugin.source ?? ''),
    )).toBe(true);
    ...
    const resolved = resolvePluginInMarketplaces(db, 'storyforge/build-test');
  ...
  it('keeps checked-in community registry entries pointed at source folders that can pack', async () => {
    ...
    const sourceSubpath = entry!.source!.replace(/^github:nexu-io\/storyforge(?:@[^/]+)?\//, '');
    expect(sourceSubpath).toBe('plugins/community/registry-starter');

    const sourceManifest = await readFile(
      new URL(`../../../${sourceSubpath}/storyforge.json`, import.meta.url),
      'utf8',
    );
    expect(JSON.parse(sourceManifest)).toMatchObject({
      name: 'community-registry-starter',
      plugin: {
        repo: expect.stringContaining('github.com/nexu-io/storyforge'),
      },
    });
  });
});
...
describe('resolvePluginInMarketplaces', () => {
  it('returns the canonical source string for a known plugin name', async () => {
    ...
    expect(resolved!.source).toBe('github:storyforge/sample-plugin');
