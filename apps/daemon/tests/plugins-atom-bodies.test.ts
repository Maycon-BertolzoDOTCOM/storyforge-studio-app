...
// pure renderer in @storyforge-app/contracts then assembles the stage
...
import { renderActiveStageBlock } from '@storyforge-app/contracts';
...
beforeEach(async () => {
  ...
  await writeFile(path.join(atomA, 'storyforge.json'), SAMPLE_MANIFEST('discovery-question-form'));
  await writeFile(path.join(atomA, 'SKILL.md'), SAMPLE_SKILL('discovery-question-form', 'Ask the user about audience.'));
  await writeFile(path.join(atomB, 'storyforge.json'), SAMPLE_MANIFEST('todo-write'));
