...
const registry: AppDirectoryRegistry = {
  packageNameByDirectory: new Map([
    ["daemon", "@storyforge-app/daemon"],
    ["web", "@storyforge-app/web"],
  ]),
};
...
test("cross-app import check rejects another app's package name", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "import type { RunRecord } from '@storyforge-app/daemon/src/server.ts';",
      "const mod = await import('@storyforge-app/daemon');",
      "const legacy = require('@storyforge-app/daemon/dist/cli.js');",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects another app's package name in require.resolve", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "const daemonManifest = require.resolve('@storyforge-app/daemon/package.json');",
    registry,
  );

  assert.equal(violations.length, 1);
  assert.equal(violations[0]?.specifier, "@storyforge-app/daemon/package.json");
  assert.equal(violations[0]?.targetApp, "daemon");
  assert.equal(violations[0]?.lineNumber, 1);
});
...
test("cross-app import check rejects createRequire-based cross-app resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "const directDaemonManifest = createRequire(import.meta.url).resolve('@storyforge-app/daemon/package.json');",
      "const aliasDaemonManifest = makeRequire(import.meta.url).resolve('@storyforge-app/daemon/package.json');",
      "const nodeRequire = nodeModule.createRequire(import.meta.url);",
      "const daemonCli = nodeRequire('@storyforge-app/daemon/dist/cli.js');",
      "const daemonPackageJson = nodeRequire.resolve('@storyforge-app/daemon/package.json');",
    ].join("\n"),
    registry,
  );
  ...
  assert.deepEqual(
    ...
    [
      "@storyforge-app/daemon/package.json",
      "@storyforge-app/daemon/package.json",
      "@storyforge-app/daemon/dist/cli.js",
      "@storyforge-app/daemon/package.json",
    ],
  );
  assert.deepEqual(
...
test("cross-app import check rejects CommonJS node:module namespace createRequire resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      'nodeRequire.resolve("@storyforge-app/daemon/package.json");',
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects cross-app imports from app-owned mjs entrypoints", () => {
  ...
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { startDaemon } from '@storyforge-app/daemon/src/server.ts';",
    registry,
  );
...
test("cross-app import check allows packages, same-app relatives, and externals", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "import type { ChatRunEvent } from '@storyforge-app/contracts';",
...
test("cross-app import check ignores quoted snippets and comments", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "const snippet = \"import { x } from '@storyforge-app/daemon/src/server.ts';\";",
      "// import { y } from '@storyforge-app/daemon/src/server.ts';",
      "/*",
      "require('@storyforge-app/daemon/dist/cli.js');",
      "*/",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check allows allowlisted packaged -> desktop main export", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { applyOsLocaleSwitch, createSplashWindow } from '@storyforge-app/desktop/main';",
    { packageNameByDirectory: new Map([["packaged", "@storyforge-app/packaged"], ["desktop", "@storyforge-app/desktop"]]) },
  );
...
test("app registry loading fails loudly when an app manifest is malformed", async () => {
  const appsRoot = await mkdtemp(path.join(os.tmpdir(), "storyforge-apps-"));
